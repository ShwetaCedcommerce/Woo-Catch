	...
	public function column_profile( $item ) {
		...
		if ( ! empty( $get_profile_id_of_prod_level ) ) {
			...
			if ( ! empty( $get_catch_category_id ) ) {
				...
				echo '<b class="not_completed">' . esc_attr( __( 'Profile Not Assigned', 'woocommerce-catch-integration' ) ) . '</b>';
			}
		}
		...
		if ( $product_status || ! empty( $pro_sync_Status ) ) {
			...
			if ( ! empty( $ImportIds ) ) {
				foreach ( $ImportIds as $key => $importId ) {
					// $importId['import_id'] = '14423807';
					$filename   = $uploadDir . '/Integration_Feed' . $importId['import_id'] . '.csv';
					$error_file = $uploadDir . '/Feed' . $importId['import_id'] . '.csv';
					if ( file_exists( $filename ) ) {
							...
							while ( ( $row = fgetcsv( $handle, 0, ';' ) ) !== false ) {
								...
								if ( 'variable' == $type ) {
									...
									foreach ( $variations as $variation ) {
										...
										if ( ! empty( $data ) && isset( $data[1] ) && $data[1] == $variation['sku'] ) {

											update_post_meta( $variation_id, 'ced_catch_uploaded_product_status' . $shop_id, $data[3] );
											update_post_meta( $variation_id, 'ced_catch_uploaded_product_status_reason' . $shop_id, $data[4] );
											$r = true;
											break;
										}
									}
								} else {
									if ( ! empty( $data ) && isset( $data[1] ) && $data[1] == $item['sku'] ) {
										update_post_meta( $item['id'], 'ced_catch_uploaded_product_status' . $shop_id, $data[3] );
										update_post_meta( $item['id'], 'ced_catch_uploaded_product_status_reason' . $shop_id, $data[4] );
										$r = true;
										break;
									}
								}
						...
						$handle_error = fopen( $error_file, 'r' );
						if ( false !== $handle_error ) {
							$row_error = fgetcsv( $handle_error );
							unset( $row_error[0] );
							while ( ( $row_error = fgetcsv( $handle_error, 0, ';' ) ) !== false ) {
								$data = $row_error;
								if ( 'variable' == $type ) {
									$variations = $prod_data->get_available_variations();
									foreach ( $variations as $variation ) {
			...
			if ( false == $r ) {
				...
				if ( is_array( $ced_catch_validation_error ) && ! empty( $ced_catch_validation_error ) ) {
					...
					foreach ( $ced_catch_validation_error as $attribute => $message ) {
						echo '<li class="catch-error">' . esc_attr( $attribute + 1 ) . ' : ' . esc_attr( $message ) . '</li>';
					}
					echo '</ul><div>';
				}
			} else {
				...
				if ( 'variable' == $type ) {
					...
					if ( ! empty( $variations ) && is_array( $variations ) ) {
						foreach ( $variations as $variation ) {
							...
							$status_reason      = get_post_meta( $variation_id, 'ced_catch_uploaded_product_status_reason' . $shop_id, true );
							$status             = get_post_meta( $variation_id, 'ced_catch_uploaded_product_status' . $shop_id, true );
