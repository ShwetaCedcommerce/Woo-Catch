...
if ( ! class_exists( 'Class_Ced_Catch_Manager' ) ) {
	...
	class Class_Ced_Catch_Manager {
		...
		*/
		...
		public function prepareProductHtmlForOffer( $proIDs = array(), $shopID = '', $UpdateOrDelete = '', $isCron = false, $with_product = '' ) {
			if ( ! is_array( $proIDs ) ) {
				$proIDs = array( $proIDs );
			}
			if ( empty( $shopID ) ) {
			...
			$response                 = $this->catchProductsInstance->ced_catch_prepareDataForOffers( $proIDs, $shopID, $UpdateOrDelete, $isCron, '', $with_product );
			$uploadDir                = $baseDir . '/cedcommerce_catchuploads';
			$filename                 = str_replace( $uploadDir, '', $response );
