...
class Class_Ced_Catch_Products {
	...
	public function ced_catch_prepareDataForUploading( $proIDs = array(), $shopId = '', $Offset = 'False', $profile_id = '' ) {
		foreach ( $proIDs as $key => $value ) {
			...
			if ( 'variable' == $type ) {
				...
				foreach ( $variations as $variation ) {
					...
					$preparedData[ $variation_id ] = $this->getFormattedData( $variation_id, $shopId, $attributes, $profile_id );
					if ( ! is_array( $preparedData[ $variation_id ] ) ) {
						unset( $preparedData[ $variation_id ] );
					}
				}
			} else {
				$preparedData[ $value ] = $this->getFormattedData( $value, $shopId, '', $profile_id );
	...
	public function ced_catch_prepareDataForOffers( $proIDs = array(), $shopId = '', $UpdateOrDelete = '', $isCron = false, $Offset = 'False', $with_product = '', $profile_id = '' ) {
		...
		foreach ( $proIDs as $key => $value ) {
			...
			if ( 'variable' == $type ) {
				...
				foreach ( $variations as $variation ) {
					$attributes                    = $variation['attributes'];
					$variation_id                  = $variation['variation_id'];
					$preparedData[ $variation_id ] = $this->getFormattedDataforOffer( $variation_id, $shopId, $UpdateOrDelete, $attributes, $with_product, $profile_id );
					if ( ! is_array( $preparedData[ $variation_id ] ) ) {
						unset( $preparedData[ $variation_id ] );
					}
				}
			} else {
				$preparedData[ $value ] = $this->getFormattedDataforOffer( $value, $shopId, $UpdateOrDelete, '', $with_product, $profile_id );
				if ( ! is_array( $preparedData[ $value ] ) ) {
					unset( $preparedData[ $value ] );
				}
			}
	 ...
	 * @return void
	 */
	public function getFormattedData( $proIds = array(), $shopId = '', $attributesforVariation = '', $profile_id = '' ) {
		$profileData = $this->ced_catch_getProfileAssignedData( $proIds, $shopId, $profile_id );
		...
		if ( ! empty( $custom_description ) ) {
			$args['product-description'] = $custom_description;
		}
		...
		if ( 'variation' == $product->get_type() ) {
			...
			$custom_description = get_post_meta( $variant_parent_id, '_ced_catch_custom_description', true );
			if ( ! empty( $custom_description ) ) {
				$args['product-description'] = $custom_description;
			}
			...
			if ( 'on' != $renderDataOnGlobalSettingsVariation[ $shopId ]['ced_catch_upload_pro_as_a_simple'] ) {
				...
				$args['title']                = $product_data['name'];
			}

			$custom_title = get_post_meta( $variant_parent_id, 'ced_catch_custom_title', true );
			if ( ! empty( $custom_title ) ) {
				$args['title'] = $custom_title;
			}
		...
		$args['product-description'] = preg_replace( '#\[[^\]]+\]#', '', $args['product-description'] );

		$args['product-description'] = strip_tags( $args['product-description'], '<p><ul><li><strong><br>' );
		$args['product-description'] = nl2br( $args['product-description'] );
		$args['product-description'] = preg_replace( '#(<br */?>\s*)+#i', '<br />', $args['product-description'] );

		$args['product-description'] = str_replace( '</strong><br />', '</strong> ', $args['product-description'] );
		$args['product-description'] = str_replace( '<ul><br />', '<ul>', $args['product-description'] );
		...
		if ( ! empty( $ced_catch_remove_spacial_character ) ) {
			...
			foreach ( $ced_catch_remove_spacial_character as $rem_key => $rem_value ) {
				$args['product-description'] = str_replace( $rem_value, '', $args['product-description'] );
			}
		}
		...
		if ( ! empty( $ced_catch_validation ) && is_array( $ced_catch_validation ) ) {
			update_post_meta( $proIds, 'ced_catch_validation_error', $ced_catch_validation );
			if ( 'variation' == $product->get_type() ) {
				$variant_parent_id = $product->get_parent_id();
				update_post_meta( $variant_parent_id, 'ced_catch_validation_error', 'yes' );
			}
			return;
		}
		...
		if ( 'variation' == $product->get_type() ) {
			$variant_parent_id = $product->get_parent_id();
			update_post_meta( $variant_parent_id, 'ced_catch_validation_error', '' );
		}
		return $args;
	}
	...
	public function ced_catch_validation( $args, $proIds ) {
		...
		if ( empty( $args['contains-button-cell-batteries'] ) ) {
			$error[] = 'Contains button cell batteries is empty.';
		}

		if ( empty( $args['keywords'] ) ) {
			$error[] = 'Keywords is empty';
		}
	 ...
	 * @return void
	 */
	public function getFormattedDataforOffer( $proIds = array(), $shopId = '', $UpdateOrDelete = '', $attributesforVariation = '', $with_product = '', $profile_id = '' ) {
		$profileData = $this->ced_catch_getProfileAssignedData( $proIds, $shopId, $profile_id );
		if ( ! $this->isProfileAssignedToProduct ) {
			return;
		}
		...
		if ( 'with_product' == $with_product ) {
			$prod_args = $this->getFormattedData( $proIds, $shopId, '', $profile_id );
			if ( ! empty( $prod_args ) && is_array( $prod_args ) ) {
	...
	*
	*/
	public function ced_catch_getProfileAssignedData( $proIds, $shopId, $profile_id ) {
	...
	*/
				...
				if ( isset( $tempProfileData['default'] ) && ! empty( $tempProfileData['default'] ) && '' != $tempProfileData['default'] && ! is_null( $tempProfileData['default'] ) ) {
					...
					if ( strpos( $tempProfileData['metakey'], 'umb_pattr_' ) !== false ) {
						...
						if ( $_product->get_type() == 'variation' ) {
							...
							if ( isset( $attributes[ 'attribute_pa_' . $wooAttribute ] ) ) {
	...
	public function uploadToCatch( $parameters, $shopId = '', $uploadType = '', $isCron = false ) {
