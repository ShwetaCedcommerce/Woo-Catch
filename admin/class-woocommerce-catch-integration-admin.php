	...
	public function ced_catch_process_bulk_action() {
		...
		if ( $check_ajax ) {
			...
			if ( is_array( $product_ids ) && ! empty( $product_ids ) ) {
				...
				if ( 'upload_offer_with_product' == $operation ) {
					$prodIDs = $product_ids;
					if ( ! empty( $prodIDs ) && is_array( $prodIDs ) ) {
						foreach ( $prodIDs as $key => $prodID ) {
							$get_product_detail = $ced_catch_manager->prepareProductHtmlForOffer( $prodID, $shop_id, 'UPDATE', '', 'with_product' );
						}
					}
					if ( isset( $get_product_detail['import_id'] ) ) {
						echo json_encode(
							array(
								'status'  => 200,
								'message' => 'Offer with Product(s) File Uploaded Successfully .',
								'prodid'  => $prodIDs,
							)
						);
						die;
				...
				} elseif ( 'upload_product' == $operation ) {
					$prodIDs = $product_ids;
					if ( ! empty( $prodIDs ) && is_array( $prodIDs ) ) {
						foreach ( $prodIDs as $key => $prodID ) {
							$get_product_detail = $ced_catch_manager->prepareProductHtmlForUpload( $prodID, $shop_id );
						}
					}
					if ( isset( $get_product_detail['import_id'] ) ) {
						echo json_encode(
					...
					if ( ! empty( $prodIDs ) && is_array( $prodIDs ) ) {
						foreach ( $prodIDs as $key => $prodID ) {
							$get_product_detail = $ced_catch_manager->prepareProductHtmlForOffer( $prodID, $shop_id, 'UPDATE' );
						}
					}
					if ( isset( $get_product_detail['import_id'] ) ) {
						echo json_encode(
					...
					if ( ! empty( $prodIDs ) && is_array( $prodIDs ) ) {
						foreach ( $prodIDs as $key => $prodID ) {
							$get_product_detail = $ced_catch_manager->prepareProductHtmlForOffer( $prodID, $shop_id, 'DELETE' );
						}
					}
					if ( isset( $get_product_detail['import_id'] ) ) {
						echo json_encode(
				...
				} elseif ( 'upload_combine' == $operation ) {
					$prodIDs = $product_ids;

					if ( ! empty( $prodIDs ) && is_array( $prodIDs ) ) {
						foreach ( $prodIDs as $key => $prodID ) {
							$get_product_detail = $ced_catch_manager->prepareProductHtmlForOfferWithProduct( $prodID, $shop_id, 'UPDATE' );
						}
					}
					if ( isset( $get_product_detail['import_id'] ) ) {
						echo json_encode(
